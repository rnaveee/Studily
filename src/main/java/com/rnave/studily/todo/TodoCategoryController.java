...
public class TodoCategoryController {
    ...
    public TodoCategoryController(TodoCategoryRepository categoryRepository, CurrentUser currentUser) {
    ...
    public List<TodoCategoryDto> list() {
        return categoryRepository.findByUserIdOrderByNameAsc(currentUser.id())
                .stream().map(TodoCategoryDto::from).toList();
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    @Transactional
    public TodoCategoryDto create(@Valid @RequestBody TodoCategoryRequest req) {
        ...
        return TodoCategoryDto.from(categoryRepository.save(category));
    }

    @PutMapping("/{id}")
    @Transactional
    public TodoCategoryDto update(@PathVariable Long id, @Valid @RequestBody TodoCategoryRequest req) {
        TodoCategory category = requireOwned(id);
        String name = req.name().trim();
    ...
    private TodoCategory requireOwned(Long id) {
        return categoryRepository.findByIdAndUserId(id, currentUser.id())
                .orElseThrow(() -> new NotFoundException("Category not found"));
    }

    public record TodoCategoryDto(Long id, String name, String color) {
        public static TodoCategoryDto from(TodoCategory c) {
            return new TodoCategoryDto(c.getId(), c.getName(), c.getColor());
        }
    }
